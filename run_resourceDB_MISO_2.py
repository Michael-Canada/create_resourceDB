...
# old nat gas coefficients reader
...
# Reading nat gas coefficients from GS:
if True:
    import gcsfs
    ...
    directories = fs.ls("gs://marginalunit-placebo/nat-gas-prod/")
    if directories:
        latest_directory = sorted(directories)[-1]

        # List all files in the specified directory
        # files = fs.ls("gs://marginalunit-placebo/nat-gas-prod/20241219T0000Z/")
        files = fs.ls(latest_directory)
        ...
        if files:
            ...
            # List all files in the specified directory
            # files = fs.ls(
            #     "gs://marginalunit-placebo/nat-gas-prod/20241219T0000Z/2024-12-19T10:41:54.559628+00:00/"
            # )
            file_content = fs.ls(latest_file)
            if file_content:
                # Read the file content
                with fs.open(file_content[0], "rb") as f:
                    coefficients_new = pd.read_parquet(f)
                with fs.open(file_content[1], "rb") as f:
                    intercepts = pd.read_parquet(f)
                    intercepts.set_index("plant_name", inplace=True)
                    intercepts.drop(columns=["plant_id", "as_of"], inplace=True)
                    intercepts.rename(columns={"value": "Value"}, inplace=True)
                with fs.open(file_content[2], "rb") as f:
                    stats = pd.read_parquet(f)
                    stats_mae_90_percentile = stats.mae.quantile(0.9)
                    stats_correlation_10_percentile = stats.correlation.quantile(0.1)

    if "michael.simantov" in sys.path[0]:
        NG_hub_loc_info = pd.read_csv(
            "/Users/michael.simantov/Documents/generator_gas_prices/NG_hub_loc_info.csv"
        )
    else:
        NG_hub_loc_info = pd.read_csv("NG_hub_loc_info.csv")
...
def find_files(directory):
    ...
    for file in os.listdir(directory):
        ...
        if file.startswith(f"all_knowledge_df_Jan_20{ISO.lower()}"):
            files.append(file)
    return files
...
six_months_ago = datetime.now() - timedelta(days=6*30)  # Approximate 6 months as 180 days
stateestimator_date_orig = six_months_ago.strftime('miso_se_%Y%m%d')
stateestimator_date = stateestimator_date_orig[:-2] + "32"

obsolete = df_g[
    (
        # ((df_g.latest_appearance <= "miso_se_20230832") | (df_g.latest_appearance.isnull()))
        (df_g.latest_appearance <= stateestimator_date)   #z1z2z3
        | (df_g.latest_appearance.isnull())
    )
    ...
    if ISO == "MISO":
        if True:
            df_p = pd.read_gbq(
                ...
                FROM `vocal-door-162221.pr_forecast.miso-se-generator-20250311`
                GROUP BY uid
        ...
        df_p = pd.read_gbq(
            ...
            APPROX_QUANTILES(pmax, 100)[OFFSET(99)] as pmax_99_perc,
            MAX(pmax) as pmax_max,
            FROM `vocal-door-162221.pr_forecast.spp-se-generator-20250311`
            GROUP BY uid
            ORDER BY pmax_median DESC
            """
        )
...
if ISO == "MISO":
    ...
    # Adding Steven's found generators, 2025-02-19
    # df_map = df_map.append(
    #     {
    #         "generator_name": "TREASURE CADDO_WIND",
    #         "eia_utility_id": 54945,
    #         "eia_plant_code": 64047,
    #         "eia_gen_id": "46002",
    #     },
    #     ignore_index=True,
    # )
    # df_map = df_map.append(
    #     {
    #         "generator_name": "MATO     7COWBOY",
    #         "eia_utility_id": 65148,
    #         "eia_plant_code": 65971,
    #         "eia_gen_id": "WT1",
    #     },
    #     ignore_index=True,
    # )
    # df_map = df_map.append(
    #     {
    #         "generator_name": "TREASURE WHITROCKE_WIND",
    #         "eia_utility_id": 63961,
    #         "eia_plant_code": 64341,
    #         "eia_gen_id": "WRE",
    #     },
    #     ignore_index=True,
    # )
    # df_map = df_map.append(
    #     {
    #         "generator_name": "OKLAUNIO DIVERSION",
    #         "eia_utility_id": 49893,
    #         "eia_plant_code": 67601,
    #         "eia_gen_id": "DIV01",
    #     },
    #     ignore_index=True,
    # )
    ...
    # df_map = df_map.append(
    #     {
    #         "generator_name": "GREATPLA KIOWACT_SOLAR",
    #         "eia_utility_id": 65656,
    #         "eia_plant_code": 66642,
    #         "eia_gen_id": "USKWA",
    #     },
    #     ignore_index=True,
    # )
    # df_map = df_map.append(
    #     {
    #         "generator_name": "TONKAWA  WHITROCKW_WIND",
    #         "eia_utility_id": 63960,
    #         "eia_plant_code": 64340,
    #         "eia_gen_id": "WRW",
    #     },
    #     ignore_index=True,
    # )
    # df_map = df_map.append(
    #     {
    #         "generator_name": "ROARK    ELKCTY2_FPLP",
    #         "eia_utility_id": 56683,
    #         "eia_plant_code": 57346,
    #         "eia_gen_id": "2",
    #     },
    #     ignore_index=True,
    # )
    # df_map = df_map.append(
    #     {
    #         "generator_name": "MACON345 LITTLEBLU_WF",
    #         "eia_utility_id": 63956,
    #         "eia_plant_code": 64336,
    #         "eia_gen_id": "WFW",
    #     },
    #     ignore_index=True,
    # )
    # df_map = df_map.append(
    #     {
    #         "generator_name": "HOLT_CO  THWE2",
    #         "eia_utility_id": 49893,
    #         "eia_plant_code": 62956,
    #         "eia_gen_id": "GEN1",
    #     },
    #     ignore_index=True,
    # )
...
df_map = df_map.append(
    {
        "generator_name": "TVA_DECA DEC_05_ST",
        "eia_utility_id": 2929,
        "eia_plant_code": 55292,
        "eia_gen_id": "STG1",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "5COUNCE PCA",
        "eia_utility_id": 14362,
        "eia_plant_code": 50296,
        "eia_gen_id": "GEN1",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "ADDIS_SO UN_1",
        "eia_utility_id": 17650,
        "eia_plant_code": 55267,
        "eia_gen_id": "712",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "LAGCK_CC LCC_03_ST",
        "eia_utility_id": 18642,
        "eia_plant_code": 7845,
        "eia_gen_id": "STG1",
    },
    ignore_index=True,
...
df_map = df_map.append(
    {
        "generator_name": "LAGCK_CC LCC_01_CT",
        "eia_utility_id": 18642,
        "eia_plant_code": 7845,
        "eia_gen_id": "CTG1",
    },
...
df_map = df_map.append(
    {
        "generator_name": "LAGCK_CC LCC_02_CT",
        "eia_utility_id": 18642,
        "eia_plant_code": 7845,
        "eia_gen_id": "CTG2",
    },
...
df_map = df_map.append(
    {
        "generator_name": "2WATAUGA WTH_01",
        "eia_utility_id": 18642,
        "eia_plant_code": 3418,
        "eia_gen_id": "1",
    },
...
df_map = df_map.append(
    {
        "generator_name": "CHKMAUGA CHH_01",
        "eia_utility_id": 18642,
        "eia_plant_code": 3398,
        "eia_gen_id": "1",
    },
...
df_map = df_map.append(
    {
        "generator_name": "CHKMAUGA CHH_02",
        "eia_utility_id": 18642,
        "eia_plant_code": 3398,
        "eia_gen_id": "2",
    },
...
df_map = df_map.append(
    {
        "generator_name": "TIMSFORD TFH_01",
        "eia_utility_id": 18642,
        "eia_plant_code": 3417,
        "eia_gen_id": "1",
    },
...
df_map = df_map.append(
    {
        "generator_name": "2WATAUGA WTH_02",
        "eia_utility_id": 18642,
        "eia_plant_code": 3418,
        "eia_gen_id": "2",
    },
...
df_map = df_map.append(
    {
        "generator_name": "5DUKMAR1 MCT_01",
        "eia_utility_id": 18642,
        "eia_plant_code": 55232,
        "eia_gen_id": "CT1",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "5DUKMAR1 MCT_03",
        "eia_utility_id": 18642,
        "eia_plant_code": 55232,
        "eia_gen_id": "CT3",
    },
...
df_map = df_map.append(
    {
        "generator_name": "5DUKMAR1 MCT_02",
        "eia_utility_id": 18642,
        "eia_plant_code": 55232,
        "eia_gen_id": "CT2",
    },
...
df_map = df_map.append(
    {
        "generator_name": "5DUKMAR1 MCT_04",
        "eia_utility_id": 18642,
        "eia_plant_code": 55232,
        "eia_gen_id": "CT4",
    },
...
df_map = df_map.append(
    {
        "generator_name": "5DUKMAR1 MCT_06",
        "eia_utility_id": 18642,
        "eia_plant_code": 55232,
        "eia_gen_id": "CT6",
    },
...
df_map = df_map.append(
    {
        "generator_name": "5DUKMAR1 MCT_05",
        "eia_utility_id": 18642,
        "eia_plant_code": 55232,
        "eia_gen_id": "CT5",
    },
...
df_map = df_map.append(
    {
        "generator_name": "5DUKMAR1 MCT_08",
        "eia_utility_id": 18642,
        "eia_plant_code": 55232,
        "eia_gen_id": "CT8",
    },
...
df_map = df_map.append(
    {
        "generator_name": "5DUKMAR1 MCT_07",
        "eia_utility_id": 18642,
        "eia_plant_code": 55232,
        "eia_gen_id": "CT7",
    },
...
df_map = df_map.append(
    {
        "generator_name": "CHKMAUGA CHH_03",
        "eia_utility_id": 18642,
        "eia_plant_code": 3398,
        "eia_gen_id": "3",
    },
...
df_map = df_map.append(
    {
        "generator_name": "CHKMAUGA CHH_04",
        "eia_utility_id": 18642,
        "eia_plant_code": 3398,
        "eia_gen_id": "4",
    },
...
df_map = df_map.append(
    {
        "generator_name": "6HARTWE1 U-5",
        "eia_utility_id": 19375,
        "eia_plant_code": 754,
        "eia_gen_id": "5",
    },
    ignore_index=True,
...
df_map = df_map.append(
    {
        "generator_name": "CORDHULL CORD_03",
        "eia_utility_id": 19462,
        "eia_plant_code": 3423,
        "eia_gen_id": "3",
    },
    ignore_index=True,
...
df_map = df_map.append(
    {
        "generator_name": "CORDHULL CORD_01",
        "eia_utility_id": 19462,
        "eia_plant_code": 3423,
        "eia_gen_id": "1",
    },
...
df_map = df_map.append(
    {
        "generator_name": "J_P_PRST JPER_01",
        "eia_utility_id": 19462,
        "eia_plant_code": 3428,
        "eia_gen_id": "1",
    },
...
df_map = df_map.append(
    {
        "generator_name": "CORDHULL CORD_02",
        "eia_utility_id": 19462,
        "eia_plant_code": 3423,
        "eia_gen_id": "2",
    },
...
df_map = df_map.append(
    {
        "generator_name": "ALLATNDM U-ALTNDM",
        "eia_utility_id": 27813,
        "eia_plant_code": 760,
        "eia_gen_id": "1",
    },
    ignore_index=True,
...
df_map = df_map.append(
    {
        "generator_name": "5RIDGELY LKS",
        "eia_utility_id": 50123,
        "eia_plant_code": 65445,
        "eia_gen_id": "RIGPV",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "2CLYDE CONCEPTN_WF",
        "eia_utility_id": 55735,
        "eia_plant_code": 56537,
        "eia_gen_id": "1",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "4OSAGEWF OSAGE_WF",
        "eia_utility_id": 59380,
        "eia_plant_code": 58683,
        "eia_gen_id": "1",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "PRVDENCE PVS",
        "eia_utility_id": 60125,
        "eia_plant_code": 60337,
        "eia_gen_id": "PROV",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "SELMER_1 SN1",
        "eia_utility_id": 60331,
        "eia_plant_code": 60555,
        "eia_gen_id": "SELMI",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "SELMER_2 SN2",
        "eia_utility_id": 60332,
        "eia_plant_code": 60556,
        "eia_gen_id": "SELM2",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "MULBERR_ SMS",
        "eia_utility_id": 60453,
        "eia_plant_code": 59184,
        "eia_gen_id": "PV1",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "WILDBERY WBS",
        "eia_utility_id": 60496,
        "eia_plant_code": 62775,
        "eia_gen_id": "WILDB",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "CMBLD_SO CLS",
        "eia_utility_id": 61528,
        "eia_plant_code": 61924,
        "eia_gen_id": "CUMB",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "MCKELLAR MKS",
        "eia_utility_id": 64915,
        "eia_plant_code": 65629,
        "eia_gen_id": "SRMCK",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "BELLBUKL BBS",
        "eia_utility_id": 64916,
        "eia_plant_code": 65630,
        "eia_gen_id": "SRBEL",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "RUSSVL RVS",
        "eia_utility_id": 65743,
        "eia_plant_code": 66818,
        "eia_gen_id": "RUSVL",
    },
    ignore_index=True,
)
...
df_map = df_map.append(
    {
        "generator_name": "MILLINGT MLS_02_NEW",
        "eia_utility_id": 66699,
        "eia_plant_code": 68326,
        "eia_gen_id": "MILL2",
    },
    ignore_index=True,
)
...
#remove known duplicates that come from df_p (the big query):
dfm = dfm.drop('NEW_MAD  NMD2')
dfm = dfm.drop('NEW_MAD  NMD1')
dfm = dfm.drop('MONTPS   CTG1A')
dfm = dfm.drop('MONTPS   CTG1B')
dfm = dfm.drop('MONTPS   STG1C')
dfm = dfm.drop('5DOUGLAS DOUGLAS04')
dfm = dfm.drop('5DOUGLAS DOUGLAS01')
dfm = dfm.drop('5DOUGLAS DOUGLAS02')
dfm = dfm.drop('5DOUGLAS DOUGLAS03')
dfm = dfm.drop('5CHER_HY CRH_01')
dfm = dfm.drop('5CHER_HY CRH_02')
dfm = dfm.drop('5CHER_HY CRH_03')
dfm = dfm.drop('5CHER_HY CRH_04')
...
# read the bid files
# GAS_bids_file_name = "/Users/michael.simantov/Documents/supply_curve_building/Found_predicted_bids_12_clean_20250219.csv"
...
GAS_bids_to_use = GAS_predicted_bids[GAS_predicted_bids.GENERATOR.isin(dfm_2.index)][
    ["GENERATOR", "UNIT", "PLANT_ID", "new_gas_multiplier", "new_gas_intercept"]
]
GAS_bids_to_use.set_index("GENERATOR", inplace=True)


HYDRO_bids_file_name = "/Users/michael.simantov/Documents/supply_curve_building/Found_predicted_bids_hydro_clean_20241211.csv"
HYDRO_predicted_bids = pd.read_csv(HYDRO_bids_file_name)
HYDRO_bids_to_use = HYDRO_predicted_bids[
    HYDRO_predicted_bids.GENERATOR.isin(dfm_2.index)
][["GENERATOR", "UNIT", "PLANT_ID", "new_gas_multiplier", "new_gas_intercept"]]
HYDRO_bids_to_use.set_index("GENERATOR", inplace=True)

hub_symbol_to_name = NG_hub_loc_info.set_index("mv_symbol")["hub_name"].to_dict()

# dfm_2 = dfm_2.drop_duplicates()
dfm_2 = dfm_2.reset_index().drop_duplicates().set_index("uid")
...
for row in dfm_2.reset_index().itertuples():
    ...
    if row.uid in GAS_bids_to_use.index:
        lines = GAS_bids_to_use.loc[[row.uid], :]
        for this_line in lines.itertuples():
            if (
                ((this_line.Index.lower() == row.uid.lower())  or (this_line.Index.lower().replace('_', '').replace('-', '') == row.uid.lower().replace('_', '').replace('-', '')))
                and ((this_line.UNIT.lower() == row.epa_generator_id.lower()) or (this_line.UNIT.lower().replace('_', '').replace('-', '') == row.epa_generator_id.lower().replace('_', '').replace('-', '')))
    ...
    elif row.uid in HYDRO_bids_to_use.index:
        lines = HYDRO_bids_to_use.loc[[row.uid], :]
        for this_line in lines.itertuples():
            if (
                this_line.Index.lower() == row.uid.lower()
                and this_line.UNIT.lower() == row.epa_generator_id.lower()
                and this_line.PLANT_ID == row.eia_plant_code
                and row.energy_source_code in ["WAT"]
            ):
                dfm_2.loc[row.uid, "bid_coef"] = this_line.new_gas_multiplier
                dfm_2.loc[row.uid, "bid_intercept"] = this_line.new_gas_intercept
    ...
    if row.plant_name in coefficients_new.plant_name.unique():
        ...
        # if the success metrics of the nat gas hub matching are not good enough then do not use them
        gas_hub_matching_success = {
            "MAE": stats[stats.plant_name == row.plant_name].mae,
            "correlation": stats[stats.plant_name == row.plant_name].correlation,
        }
        if (gas_hub_matching_success["MAE"].values[0] > stats_mae_90_percentile) or (
            gas_hub_matching_success["correlation"].values[0]
            < stats_correlation_10_percentile
        ):
            pass  # do not use the nat gas coefficients
        else:

            relevant_row = coefficients_new[
                coefficients_new["plant_name"] == row.plant_name
            ].copy()
            relevant_row.loc[:, "hub_name"] = relevant_row["price_symbol"].apply(
                lambda x: hub_symbol_to_name.get(x, "")
            )
            relevant_row.drop(
                columns=["price_symbol", "plant_id", "plant_name", "as_of"],
                inplace=True,
            )
            relevant_row.set_index("hub_name", inplace=True)

            # gas_hubs_and_coefs_for_this_generator = coefficients[row.plant_name]
            current_supplier_hub_data = relevant_row.to_dict()

            # only keep hubs with coeff larger than 0.001
            current_supplier_hub_data["coefficients"] = {
                k: v
                for k, v in current_supplier_hub_data["coefficient"].items()
                if v >= 0.001
            }

            del current_supplier_hub_data["coefficient"]
    ...
    elif category_mapping.get(row.energy_source_code, "other") in {"hydro"}:
        if dfm_2.loc[row.uid, "bid_intercept"] > -999:
            offer_curve = {
                "type": "DIRECT",
                "min_gen_cost": dfm_2.loc[row.uid, "bid_intercept"],
                "blocks": [
                    {"quantity": pmax, "price": dfm_2.loc[row.uid, "bid_intercept"]}
                ],
            }
        else:
            offer_curve = {
    ...
    #     }
        ...
        offer_curve = {
            "type": "DIRECT",
            # "min_gen_cost": _DEFAULT_PRICE.get(key, 50.0),
            "min_gen_cost": _DEFAULT_PRICE.get(key, 50.0)*10.0,
            "blocks": [{"quantity": pmax, "price": _DEFAULT_PRICE.get(key, 50.0)}],
        }
