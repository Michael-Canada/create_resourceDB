...
def preprocess(path_train_data, path_test_data, target_title = 'y', raw_training_data_with_target = None, raw_testing_data = None, DEBUGGING = True):
    ...
    # raw_training_data = raw_training_data.drop(columns = 'jv_group')
    # raw_training_data = raw_training_data.drop(columns = 'storage_capacity')
    # raw_training_data = raw_training_data.drop(columns = 'end_date')
    # raw_training_data = raw_training_data.drop(columns = 'start_date')
    
    # raw_training_data = raw_training_data[['resource_name', 'pmin', 'winter_capacity', 'summer_capacity', 'min_up_time', 'ramp_rate', 'min_down_time']].copy()
    # raw_testing_data = raw_testing_data[['resource_name', 'pmin', 'winter_capacity', 'summer_capacity', 'min_up_time', 'ramp_rate', 'min_down_time']].copy()
...
def select_features(Chosen_model, X_train, y, X_test):
    ...
    model = SelectFromModel(Chosen_model, max_features=4)
