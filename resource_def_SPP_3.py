...
with open("/Users/michael.simantov/Documents/resources_SPP_epa.json", "r") as fdesc:
...
df_map_spp = pd.merge(df_map_spp, df_eia_spp, left_on=['eia_plant_code', 'eia_gen_id'], right_on=['plant_id', 'generator_id'], how='left')
...
df_map_spp['agg'] = df_map_spp.groupby('generator_name')['net_summer_capacity_mw'].transform('sum')
...
dfm = pd.merge(
    dfm,
    df_eia_spp[~df_eia_spp.plant_id.isnull()].set_index(["plant_id", "generator_id"], verify_integrity=True),
    left_on=["eia_plant_code", "eia_gen_id"],
    right_index=True,
    how="left"
)
