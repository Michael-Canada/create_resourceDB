...
if GO_TO_GCLOUD:
    df_g_miso = _get_dfm(f"https://api1.marginalunit.com/reflow/{COLLECTION}/generators")
...
df_map_miso = df_map_miso.append({'generator_name': 'ORENTDRT ORENT_UNIT2', 'eia_utility_id': 12341, 'eia_plant_code': 61077, 'eia_gen_id': 2}, ignore_index=True)
...
with open("/Users/michael.simantov/Documents/mu-power-plant-economic-modeling/economic_modeling/resourcedb/2024-01-16/resources_epa.json", "r") as fdesc:
    epa_eia_mapping = json.load(fdesc)

records = [
    {"orispl": r["orispl"], "epa_generator_id": r["generator_id"], "eia_unit_id": u["eia_unit"]["generator_id"]}
...
df_map_miso = pd.merge(df_map_miso, df_eia_miso, left_on=['eia_plant_code', 'eia_gen_id'], right_on=['plant_id', 'generator_id'], how='left')
...
df_map_miso['agg'] = df_map_miso.groupby('generator_name')['net_summer_capacity_mw'].transform('sum')
...
dfm = pd.merge(
    dfm,
    df_eia_miso[~df_eia_miso.plant_id.isnull()].set_index(["plant_id", "generator_id"], verify_integrity=True),
    left_on=["eia_plant_code", "eia_gen_id"],
    right_index=True,
    how="left"
)
...
dfm.loc[~dfm.net_summer_capacity_mw.isnull() & ~dfm.pmax_99_perc.isnull() & (dfm.net_summer_capacity_mw < dfm.pmax_99_perc) & (dfm.net_summer_capacity_mw > dfm.pmin_median), "pmax_99_perc"] = dfm.net_summer_capacity_mw
...
dfm.loc[~dfm.net_summer_capacity_mw.isnull() & dfm.pmax_99_perc.isnull(), "pmax_99_perc"] = dfm.net_summer_capacity_mw
...
dfm_1.loc[:, ["fitting_param_x", "fitting_param_intercept"]] = dfm_1.apply(
    lambda r: avg_heat_rate.loc[(r.energy_source_code, r.prime_mover_code)]
...
for key, value in default_hr.items():
    ...
    dfm_1.loc[
        (dfm_1.energy_source_code == energy_source) & (dfm_1.prime_mover_code == prime_mover),
        ["fitting_param_x", "fitting_param_intercept"]
    ].fillna(
        {"fitting_param_x": fitting_param_x, "fitting_param_intercept": fitting_param_intercept}, inplace=True
    )
...
if False:
    ...
    this_dfm_3 = dfm_2.query('energy_source_code == "NG" and prime_mover_code == "GT"').copy()
    ...
    plt.hist(df_3[df_3['min_max_forecast'] <= .5]['min_max_actual'].values, bins=100)
...
if DEBUG_ON:
    ...
    for row in dfm_2.itertuples():
        if row.Index in df.index:
            ...
            if actual_pg_max > dfm_Pmax:   # this is the cases that we want to increase pmax_Forecast
            ...
            # if orange_line * 2 < dfm_Pmax:
                ...
                ax = graphs['fcst_pg'].plot(color='green', linewidth=1)  # Plot the first series and get the axis
                ...
                plt.title("case " + str(ccc) + ':  ' + str(dfm_2.loc[row.Index][['eia_plant_code','plant_name','eia_gen_id']].values))

                # set dfm_2's pmax_99_perc to the actual_pg_max
                print(dfm_2.loc[row.Index][['plant_name', 'eia_plant_code','eia_gen_id','pmax_max','pmax_median','pmax_99_perc']])

                print(df.loc[row.Index][['actual_pg', 'fcst_pg', 'pmin_Actual','pmax_Actual', 'pmax_Forecast', 'pmin_Forecast']][200:205])
                print(dfm_2.loc[row.Index][['pmax_max','pmax_99_perc','pmin_median']])
                print(f'pmax_Actual: {df.loc[row.Index]["pmax_Actual"].max()}, initial pmax_Forecast was {dfm_2.loc[row.Index]["pmax_max"]} and I forced it to {dfm_2.loc[row.Index]["pmax_99_perc"]}')
...
if False:   # the following yields an unclear error
    ...
    dfm_2.loc[dfm_2.uid == 'ORENTDRT ORENT_UNIT2', 'generator_name'] = 'ORENTDRT ORENT_UNIT2'
...
dfm_2 = dfm_2[~dfm_2.operating_month.isnull() | ~dfm_2.planned_operation_month.isnull()].copy()
...
category_mapping = {
    'AB': 'coal', 'BFG': 'biogas', 'BIT': 'coal', 'BLQ': 'biomass', 'DFO': 'oil', 'JF': 'oil',
    'KER': 'oil', 'LFG': 'biogas', 'LIG': 'coal', 'MSW': 'waste', 'MWH': 'hydro', 'NG': 'gas',
    'NUC': 'nuclear', 'OBG': 'biogas', 'OG': 'oil', 'PC': 'coal', 'PUR': 'other', 'RC': 'coal',
    'RFO': 'oil', 'SGC': 'gas', 'SUB': 'coal', 'SUN': 'solar', 'WAT': 'hydro', 'WC': 'coal',
    'WDS': 'biomass', 'WH': 'waste heat', 'WND': 'wind'
}

# Sample lists
list1 = [
    ('AB', 'ST'), ('BFG', 'ST'), ('BIT', 'ST'), ('BLQ', 'ST'), ('DFO', 'CA'), ('DFO', 'CT'),
    ('DFO', 'GT'), ('DFO', 'IC'), ('DFO', 'ST'), ('JF', 'GT'), ('KER', 'GT'), ('LFG', 'CT'),
    ('LFG', 'GT'), ('LFG', 'IC'), ('LFG', 'ST'), ('LIG', 'ST'), ('MSW', 'ST'), ('MWH', 'BA'),
    ('MWH', 'FW'), ('NG', 'CA'), ('NG', 'CS'), ('NG', 'CT'), ('NG', 'FC'), ('NG', 'GT'),
    ('NG', 'IC'), ('NG', 'ST'), ('NUC', 'ST'), ('OBG', 'IC'), ('OG', 'CA'), ('OG', 'OT'),
    ('OG', 'ST'), ('PC', 'ST'), ('PUR', 'ST'), ('RC', 'ST'), ('RFO', 'ST'), ('SGC', 'CA'),
    ('SGC', 'CT'), ('SUB', 'ST'), ('SUN', 'PV'), ('WAT', 'HY'), ('WAT', 'PS'), ('WC', 'ST'),
    ('WDS', 'ST'), ('WH', 'OT'), ('WH', 'ST'), ('WND', 'WS'), ('WND', 'WT')
]
list2 = [
    ('DFO', 'GT'), ('LIG', 'ST'), ('MWH', 'BA'), ('NG', 'CA'), ('NG', 'CS'),
    ('NG', 'CT'), ('NG', 'GT'), ('NG', 'ST'), ('NUC', 'ST'), ('SUB', 'ST'),
    ('SUN', 'PV'), ('WAT', 'HY'), ('WDS', 'ST'), ('WND', 'WT')
]
...
dfm_2['category'] = dfm_2.apply(lambda row: results.get((row.energy_source_code, row.prime_mover_code), 'other'), axis=1)
...
dfm_2 = dfm_2.drop(columns=['operating_month', 'latitude', 'longitude', 'eia_utility_id', 'eia_plant_code', 'plant_state', 'plant_name'])
...
for category in ML_modeling_results.keys():
    ...
    raw_training_data_with_target = pd.read_csv(f'raw_training_data_with_target_{category}.csv')
    ...
    processed_training_data, processed_testing_data, target, target_bias, target_std, numeric_indices, all_titles_of_features = economic_model_ML_funcs.preprocess(None, None, 'pmin_median', raw_training_data_with_target, raw_testing_data, False)
    ...
    result_XGB, result_linear = economic_model_ML_funcs.check_model_performance(selected_train_data_XGB, selected_train_data_linear, scores_reduced_XGB, scores_reduced_linear, model_XGB, model_linear, target)
    print(result_XGB)
    print(result_linear)

    economic_model_ML_funcs.report_R_squared(target, scores_reduced_linear, scores_reduced_XGB)

    prediction_linear = economic_model_ML_funcs.make_predictions('linear', target_bias, target_std, selected_test_data_linear, selected_test_data_XGB, model_XGB, model_linear)
    prediction_XGB = economic_model_ML_funcs.make_predictions('XGB', target_bias, target_std, selected_test_data_linear, selected_test_data_XGB, model_XGB, model_linear)
    ...
    print(pd.Series(model_XGB.feature_importances_, index = Chosen_features_XGBoost).sort_values(ascending=False))
...
dfm_2_copy.loc[(dfm_2.category == 'coal') & (dfm_2.pmin_median > 0.8 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.8 * dfm_2.pmax_99_perc
...
dfm_2_copy.loc[(dfm_2.category == 'coal') & (dfm_2.prime_mover_code == 'ST') & (dfm_2.pmin_median < 0.40 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.40 * dfm_2.pmax_99_perc
dfm_2_copy.loc[(dfm_2.category == 'coal') & (dfm_2.pmin_median < 0.40 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.40 * dfm_2.pmax_99_perc   #for all other prime_mover_codes
...
dfm_2_copy.loc[(dfm_2.category == 'gas') & (dfm_2.prime_mover_code == 'ST') & (dfm_2.pmin_median < 0.35 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.35 * dfm_2.pmax_99_perc
dfm_2_copy.loc[(dfm_2.category == 'gas') & (dfm_2.prime_mover_code == 'CT') & (dfm_2.pmin_median < 0.5 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.5 * dfm_2.pmax_99_perc
dfm_2_copy.loc[(dfm_2.category == 'gas') & (dfm_2.prime_mover_code == 'CA') & (dfm_2.pmin_median < 0.5 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.5 * dfm_2.pmax_99_perc
dfm_2_copy.loc[(dfm_2.category == 'gas') & (dfm_2.prime_mover_code == 'GT') & (dfm_2.pmin_median < 0.5 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.5 * dfm_2.pmax_99_perc
dfm_2_copy.loc[(dfm_2.category == 'gas') & (dfm_2.pmin_median < 0.35 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.35 * dfm_2.pmax_99_perc   #for all other prime_mover_codes
...
dfm_2_copy.loc[(dfm_2.category == 'oil') & (dfm_2.pmin_median > 0.85 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.85 * dfm_2.pmax_99_perc
...
dfm_2_copy.loc[(dfm_2.category == 'oil') & (dfm_2.prime_mover_code == 'ST') & (dfm_2.pmin_median < 0.35 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.35 * dfm_2.pmax_99_perc
dfm_2_copy.loc[(dfm_2.category == 'oil') & (dfm_2.prime_mover_code == 'CA') & (dfm_2.pmin_median < 0.5 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.5 * dfm_2.pmax_99_perc
dfm_2_copy.loc[(dfm_2.category == 'oil') & (dfm_2.prime_mover_code == 'CT') & (dfm_2.pmin_median < 0.5 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.5 * dfm_2.pmax_99_perc
dfm_2_copy.loc[(dfm_2.category == 'oil') & (dfm_2.prime_mover_code == 'GT') & (dfm_2.pmin_median < 0.5 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.5 * dfm_2.pmax_99_perc
dfm_2_copy.loc[(dfm_2.category == 'oil') & (dfm_2.pmin_median < 0.35 * dfm_2.pmax_99_perc), 'pmin_median'] = 0.35 * dfm_2.pmax_99_perc   #for all other prime_mover_codes

# renewbles and other
dfm_2_copy.loc[dfm_2.category == 'hydro', 'pmin_median'] = dfm_2_copy.loc[dfm_2.category == 'hydro', 'pmin_median'].clip(lower=0)
dfm_2_copy.loc[dfm_2.category == 'nuclear', 'pmin_median'] = dfm_2_copy.loc[dfm_2.category == 'nuclear', 'pmin_median'].clip(lower=0)
dfm_2_copy.loc[dfm_2.category == 'wind', 'pmin_median'] = 0
dfm_2_copy.loc[dfm_2.category == 'biogas', 'pmin_median'] = dfm_2_copy.loc[dfm_2.category == 'biogas', 'pmin_median'].clip(lower=0)
dfm_2_copy.loc[dfm_2.category == 'biomass', 'pmin_median'] = dfm_2_copy.loc[dfm_2.category == 'biomass', 'pmin_median'].clip(lower=0)
dfm_2_copy.loc[dfm_2.category == 'waste', 'pmin_median'] = dfm_2_copy.loc[dfm_2.category == 'waste', 'pmin_median'].clip(lower=0)
dfm_2_copy.loc[dfm_2.category == 'other', 'pmin_median'] = dfm_2_copy.loc[dfm_2.category == 'other', 'pmin_median'].clip(lower=0)
dfm_2_copy.loc[dfm_2.category == 'solar', 'pmin_median'] = dfm_2_copy.loc[dfm_2.category == 'solar', 'pmin_median'].clip(lower=0)
dfm_2_copy.loc[dfm_2.category == 'waste heat', 'pmin_median'] = dfm_2_copy.loc[dfm_2.category == 'waste heat', 'pmin_median'].clip(lower=0)
...
# use the functions in economic_model_ML_funcs to predict pmin_median. This should be done for each category that appears more than 20 times in the dataframe
for category in ML_modeling_results.keys():
    raw_testing_data_ALL = dfm_2[dfm_2.category == category].drop(columns=['fitting_param_intercept'])    # fitting_param_x
    raw_training_data_with_target_ALL = pd.read_csv(f'raw_training_data_with_target_{category}.csv')
    ...
    counts = raw_training_data_with_target_ALL.groupby(['energy_source_code', 'prime_mover_code']).size()
    ...
    for pair in counts.index:
        ...
        raw_testing_data = raw_testing_data_ALL[(raw_testing_data_ALL.energy_source_code == pair[0]) & (raw_testing_data_ALL.prime_mover_code == pair[1])]
        raw_training_data_with_target = raw_training_data_with_target_ALL[(raw_training_data_with_target_ALL.energy_source_code == pair[0]) & (raw_training_data_with_target_ALL.prime_mover_code == pair[1])]
        ...
        # Preprocess raw data, to impute NaN, scale numerical features and remove target bias
        processed_training_data, processed_testing_data, target, target_bias, target_std, numeric_indices, all_titles_of_features = economic_model_ML_funcs.preprocess(None, None, 'fitting_param_intercept', raw_training_data_with_target, raw_testing_data, False)
        if processed_training_data is None:
            continue
        ...
        # add the predicted values to the dataframe dfm_2
        dfm_2.loc[(dfm_2.category == category) & (dfm_2.energy_source_code == pair[0]) & (dfm_2.prime_mover_code == pair[1]), 'fitting_param_intercept'] = prediction_XGB
...
# use the functions in economic_model_ML_funcs to predict pmin_median. This should be done for each category that appears more than 20 times in the dataframe
for category in ML_modeling_results.keys():
    raw_testing_data_ALL = dfm_2[dfm_2.category == category].drop(columns=['fitting_param_x'])    # fitting_param_x
    raw_training_data_with_target_ALL = pd.read_csv(f'raw_training_data_with_target_{category}.csv')
    ...
    for pair in counts.index:
        ...
        # Preprocess raw data, to impute NaN, scale numerical features and remove target bias
        processed_training_data, processed_testing_data, target, target_bias, target_std, numeric_indices, all_titles_of_features = economic_model_ML_funcs.preprocess(None, None, 'fitting_param_x', raw_training_data_with_target, raw_testing_data, False)
        if processed_training_data is None:
            continue
        ...
        # add the predicted values to the dataframe dfm_2
        dfm_2.loc[(dfm_2.category == category) & (dfm_2.energy_source_code == pair[0]) & (dfm_2.prime_mover_code == pair[1]), 'fitting_param_x'] = prediction_XGB
...
if False:
    ...
    dfm_2.loc[:, ["fitting_param_x", "fitting_param_intercept"]] = dfm_2.apply(
        lambda r: avg_heat_rate.loc[(r.prime_mover_code)]
            if pd.isnull(r.fitting_param_x)
            else r[["fitting_param_x", "fitting_param_intercept"]],
        axis=1
    )
...
dfm = df.groupby(["plant_id", "plant_name", "reported_fuel_type_code"])[["total_fuel_consumption_mmbtu", "net_generation_mwh"]].sum().reset_index()
...
dfm = dfm.merge(dfm_2[['energy_source_code', 'plant_name', 'prime_mover_code', 'eia_plant_code']], 
                left_on=['reported_fuel_type_code', 'plant_name', 'plant_id'], 
                right_on=['energy_source_code', 'plant_name', 'eia_plant_code'], 
                how='left').drop(columns=['energy_source_code', 'eia_plant_code'])
...
group_stats = dfm.groupby(['reported_fuel_type_code','prime_mover_code'])[['heat_rate']].agg(['median', 'std'])
...
def replace_outliers(row):
    median_x = group_stats.loc[(row['reported_fuel_type_code'], row['prime_mover_code']), ('heat_rate', 'median')]
    std_x = group_stats.loc[(row['reported_fuel_type_code'], row['prime_mover_code']), ('heat_rate', 'std')]
...
generators_with_close_hubs = pd.read_csv("/Users/michael.simantov/Documents/generator_gas_prices/generators_with_close_hubs.csv")
...
for row in dfm_2.reset_index().itertuples():
    ...
    if "OFFSET".lower() in row.uid.lower() and not "STCHAR".lower() in row.uid.lower() and not "MONTPS".lower() in row.uid.lower():
        print(row.uid)
        cnttt += 1
        continue

    if "MKT".lower() in row.uid.lower() and not "STCHAR".lower() in row.uid.lower() and not "MONTPS".lower() in row.uid.lower():
        print(row.uid)
        cnttt += 1
        continue
    ...
    key =  (row.energy_source_code, row.prime_mover_code)
    min_up_time, min_down_time, ramp_rate = _DEFAULT_PHYSICAL_CHARACTERISTICS.get(key, (10, 10, 9999))
    ...
    if category_mapping.get(row.energy_source_code, 'other') in {"wind", "solar", "hydro"}:
        pmin = 0.0
    ...
    # Note: we should handle 'planned retirement' and 'planned operation year'
    assert not pd.isnull(row.operating_month) or not pd.isnull(row.planned_operation_month)

    if not pd.isnull(row.operating_month):
        start_date = date(int(row.operating_year), min(int(row.operating_month), 12), 1) - timedelta(days=365)
    else:
        start_date = date(int(row.planned_operation_year), int(row.planned_operation_month), 1) - timedelta(days=365)
    ...
    #    'current_supplier__hub_lon']])
        this_generator_data = generators_with_close_hubs[generators_with_close_hubs['plant_id'] == row.eia_plant_code]
    ...
    generators = [
        {
            ...
            "eia_uid": {
                "eia_id": row.eia_plant_code,
                "unit_id": row.eia_gen_id
            },
    ...
    if category_mapping.get(row.energy_source_code, 'other') in {"wind", "solar", "hydro", "nuclear"}:
        offer_curve = {
            "type": "DIRECT",
            "min_gen_cost": _DEFAULT_PRICE.get(key, 0.0),
            "blocks": [
                {"quantity": pmax, "price": _DEFAULT_PRICE.get(key, 0.0)}
            ]
        }
    # elif row.energy_source_code in {"NG", "SUB", "LIG"}:
        ...
        offer_curve = {
            ...
            "blocks": [
                {"quantity": pmax, "price": _DEFAULT_PRICE.get(key, 50.0)}
            ]
        }
...
if False:
    ...
    # Define a function to replace outliers
    def replace_outliers(row):
        median_x = group_stats.loc[(row['energy_source_code'], row['prime_mover_code']), ('fitting_param_x', 'median')]
        std_x = group_stats.loc[(row['energy_source_code'], row['prime_mover_code']), ('fitting_param_x', 'std')]
        median_intercept = group_stats.loc[(row['energy_source_code'], row['prime_mover_code']), ('fitting_param_intercept', 'median')]
        std_intercept = group_stats.loc[(row['energy_source_code'], row['prime_mover_code']), ('fitting_param_intercept', 'std')]
        
        if abs(row['fitting_param_x'] - median_x) > 1 * std_x or abs(row['fitting_param_intercept'] - median_intercept) > 1 * std_intercept:
...
if True:
    # Calculate the mean and standard deviation of the group
    # group_stats = dfm_2.groupby(['energy_source_code', 'prime_mover_code'])[['fitting_param_x', 'fitting_param_intercept']].agg(['median', 'std'])
    group_stats = dfm_2[dfm_2.fitting_param_x > 0.0].groupby(['energy_source_code', 'prime_mover_code'])[['fitting_param_x', 'fitting_param_intercept']].agg(['median', 'std'])

    # Define a function to replace outliers
    def replace_outliers(row):
        # check if (row['energy_source_code'], row['prime_mover_code']) is in group_stats
        if (row['energy_source_code'], row['prime_mover_code']) not in group_stats.index:
            return row
...
if True:
    ...
    dfm_2[(dfm_2.energy_source_code == 'NG') & (dfm_2.prime_mover_code == 'GT') ].fitting_param_x.hist(bins=50)
    plt.title("Heat Rate of NG GT")
    plt.figure()

    # plot a histogram of the heat rate of all the dfm_2 rows where the energy_source_code is 'NG' and the prime_mover_code is 'GT'
    dfm_2[(dfm_2.energy_source_code == 'NG') & (dfm_2.prime_mover_code == 'CA') ].fitting_param_x.hist(bins=50)
    plt.title("Heat Rate of NG CA")
    plt.figure()

    # plot a histogram of the heat rate of all the dfm_2 rows where the energy_source_code is 'NG' and the prime_mover_code is 'GT'
    dfm_2[(dfm_2.energy_source_code == 'NG') & (dfm_2.prime_mover_code == 'CT') ].fitting_param_x.hist(bins=50)
    plt.title("Heat Rate of NG CT")
    plt.figure()

    # plot a histogram of the heat rate of all the dfm_2 rows where the energy_source_code is 'NG' and the prime_mover_code is 'GT'
    dfm_2[(dfm_2.energy_source_code == 'NG') & (dfm_2.prime_mover_code == 'ST') ].fitting_param_x.hist(bins=50)
    plt.title("Heat Rate of ST ")

dfm_2.groupby(['energy_source_code', 'prime_mover_code'])[['pmin_median', 'pmax_99_perc']].agg(['median', 'max', 'min', 'count'])
...
with open(f"{version}/resources_MISO.json", "w") as fdesc:
...
with open(f"{version}/supply_curves_MISO.json", "w") as fdesc:
...
if False:
    ...
    df_crcl = df_crcl.groupby(['location','env_id','eia_id'])[['eia_id', 'location', 'capacity_mw']].first()

    # dfm_2 = dfm_2.groupby(['eia_utility_id', 'eia_plant_code', 'eia_gen_id'])[['eia_utility_id']].first()
    dfm_2 = dfm_2.groupby(['eia_utility_id', 'eia_plant_code', 'eia_gen_id'])[['eia_plant_code','plant_name']].first()
    ...
    df_exists_only_in_df_crcl = df_exists_only_in_df_crcl[~df_exists_only_in_df_crcl.index.isin(dfm_original__.index)]
    ...
    df_exists_in_dfm_original__but_not_dfm_2 = dfm_original__[~dfm_original__.uid.isin(dfm_2.uid)].plant_name.unique()

    # print the generators that are in df_crcl but not in dfm_2 (i.e., in CRCL but not in our system)
    print(df_exists_only_in_df_crcl[['capacity_mw','location']].query('capacity_mw > 50').sort_values('capacity_mw', ascending=False). sort_values('capacity_mw', ascending=False))
    print(df_exists_in_both. shape, df_exists_only_in_dfm_2.shape, df_exists_only_in_df_crcl.shape)
...
if False:
    ...
    [r for r in resources if r["physical_properties"]["pmin"] <= r["physical_properties"]["pmax"]]


    df[df.uid == "FERGUS2  STILLWATER_EC2"].physical_properties.iloc[0]

    df = pd.read_parquet("/Users/michael.simantov/Documents/draft/ps_engine_results/miso/first-20240423/denormalized_hub/08204d01-85a9-45ec-8226-692b610ac457 -- 2024-04-06T04:00:00-05:00 -- 2024-04-06T02:00:00-05:00.parquet")
